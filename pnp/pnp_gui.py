...
root = Tk()
root.title("PNP Curve Tracer")
root.geometry("677x343")
root.iconbitmap("pnp_icon.ico")
...
# Photo
photo = PhotoImage(file = "pnp_schematic.png")
...
entry_VEC_initial = EntryWithPlaceholder(root, "start")
entry_VEC_final = EntryWithPlaceholder(root, "stop")
entry_VEC_increment = EntryWithPlaceholder(root, "step")
...
dropdown_VEC_initial = ttk.Combobox(root, value = unit_volt, width = 4, state='readonly')
dropdown_VEC_initial.current(2)
dropdown_VEC_final = ttk.Combobox(root, value = unit_volt, width = 4, state='readonly')
dropdown_VEC_final.current(2)
dropdown_VEC_increment = ttk.Combobox(root, value = unit_volt, width = 4, state='readonly')
dropdown_VEC_increment.current(2)

# Write to a File
def write():
  outputFile = open("pnp_variables.txt", "w")   
  ...
  outputFile.write(entry_VEC_initial.get() + get_unit(dropdown_VEC_initial.get()) + '\n')
  outputFile.write(entry_VEC_final.get() + get_unit(dropdown_VEC_final.get()) + '\n')
  outputFile.write(entry_VEC_increment.get() + get_unit(dropdown_VEC_increment.get()))
  outputFile.close()
...
def checkInputs():
  ...
  isFloat(entry_VEC_initial.get())
  isFloat(entry_VEC_final.get())
  isFloat(entry_VEC_increment.get())
...
label_VEC.place(x=200, y=269)
entry_VEC_initial.place(x=235, y=269)
dropdown_VEC_initial.place(x=275, y=269)
entry_VEC_final.place(x=330, y=269)
dropdown_VEC_final.place(x=370, y=269)
entry_VEC_increment.place(x=425, y=269)
dropdown_VEC_increment.place(x=465, y=269)
